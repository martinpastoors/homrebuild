...
source(file.path("R", "SStoFLStock_v2.r"))
...
WHOM       <- SStoFLStock(inpDir,stockname="WHOM",fbar_amin=1,fbar_amax=10)
WHOM       <- FLCore::setPlusGroup(WHOM,15)
WHOM@stock <- ssb(WHOM)
FLS        <- WHOM

save(FLS, file = file.path("datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLS_WGWIDE.RData")))
...
save(MVND, file=file.path(outDir,"MVND.RData"))
load(      file=file.path(outDir,"MVND.RData"))
...
iters   <- seq(1,4000); niters <- length(iters)   #10k assessments from the mvrn draw
...
# ss.repDirs <- c(file.path(getwd(),file.path("data"   ,Base)),
#                 file.path(getwd(),file.path("data"   ,Base,"Set_newParameter",paste0("newParms",iters))))
# names(ss.repDirs) <- c("ass",iters)
ss.repDirs <- c(file.path(getwd(),file.path("data"   ,Base,"Set_newParameter",paste0("newParms",iters))))
names(ss.repDirs) <- c(iters)
...
# ii <- 3201
# for (ii in 1:niters){
for (ii in 3501:niters){
  print(ii)
  SS_doPar(parNames=parNames, parBound=parBound, myParSet=newParSet, n_parSets=ii, newsubdir = file.path("Set_newParameter"))
  WHOM          <- SStoFLStock(ss.repDirs[ii],stockname="WHOM",fbar_amin=1,fbar_amax=10)
  WHOM          <- FLCore::setPlusGroup(WHOM,15)
  FLSs[,,,,,ii] <- WHOM
  # dfSSB         <- bind_rows(dfSSB, data.frame(iter=ii,SSB=as.numeric(ssb(WHOM)[,as.character(tyr),,,,,])))
}

# FLSs_tmp          <- loadRData(      file = file.path(getwd(),"datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLSs_3500.RData")))
# FLSs[,,,,,1:3500] <- FLSs_tmp

save(FLSs, file = file.path(getwd(),"datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLSs_4000.RData")))
# load(      file = file.path(getwd(),"datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLSs_3500.RData")))
...
sel.iters <- c()
ntotobs   <- 0
bks       <- c(0,seq(300000,1300000,by=100000))
...
for(bin in seq(1,length(bks)-1)){
  if(bins$counts[bin]>0){
    ...
    print(paste(bin, 
                paste(bks[bin],bks[bin+1], sep="-"),
                nobs,
                ntotobs,
                bins$breaks[bin], 
                bins$counts[bin]))
    sel.iters <- c(sel.iters,
                   sample(dfSSB$iter[dfSSB$SSB>bks[bin] & dfSSB$SSB<=bks[bin+1]],
                          size=min(nobs, bins$counts[bin]),
                          replace=FALSE))
    # sel.iters <- c(sel.iters,sample(dfSSB$iter[dfSSB$SSB>bks[bin] & dfSSB$SSB<=bks[bin+1]],size=bins$counts[bin],replace=FALSE))
  }
}

# TEMPORARY !!!! artificially increase the number of valid samples; 4000 samples gives 981 valid iterations
sel.iters <- c(sel.iters, sample(sel.iters,size=19, replace=TRUE))

length(sel.iters)
# hist(dfSSB$SSB[dfSSB$iter %in% sel.iters],breaks=bks,plot=TRUE)
...
save(FLSs.1k, file = file.path(getwd(),"datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLSs_Clean.RData")))
load(         file = file.path(getwd(),"datasim", Base, paste0("WHOM_SS",substr(Base,5,6),"_FLSs_Clean.RData")))
