...
output <- SS_output(dir = dirSS3output, 
                    repfile = "Report.sso", 
...
data@stock.n <-
  FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", natage, units = '10^3')

# STOCK WEIGHT-AT-AGE: 
data@stock.wt <-FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", units = 'kg', 
                        rep(as.numeric(unlist(t(output$wtatage[(output$wtatage$Fleet==0)&(output$wtatage$Sex==1),-c(1:6,dim(output$wtatage)[2])]))), nyears))

# NATURAL MORTALITY: 
data@m <- FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", units = 'm', 
                  rep(as.numeric(output$MGparmAdj$NatM_uniform_Fem_GP_1), nyears))
...
data@m.spwn <- FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", units = 'prop', 
                       rep(0,nages*nyears))

# MATURITY-AT-AGE: 
data@mat <-FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", units = 'prop', 
                   rep(output$endgrowth$Age_Mat[1:nages],nyears))
...
# length(data@discards.n)
# dim(data@discards.n)
# as.data.frame(data@discards.n) %>% tidyr::pivot_wider(names_from = age, values_from = data) %>% View()
...
# length(wtatage_commercial)
# length(wtatage_commercial[output$wtatage$Fleet==1])
# dim(output$wtatage)
# as.data.frame(output$wtatage[output$wtatage$Fleet==1,]) %>% View()
# as.data.frame(output$wtatage[output$wtatage$Fleet==1,]) %>% nrow()
# as.data.frame(output$wtatage[(output$wtatage$Fleet==1)&(output$wtatage$Sex==1)&(output$wtatage$Yr<=yearlast),-c(1:6)])
...
data@stock <- FLQuant(dimnames = list(age = 'all', year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", units = 't', 
                      rep(1111,nyears)) 
...
# quantSums(stock.n(data) * stock.wt(data) * mat(data) *
#             (1 - harvest(data) * harvest.spwn(data)) *
#             exp(-m(data) * m.spwn(data)))
